...
const keys = {
    ...
    spuspg: "spus_page_",
    trn: 'tranche_',
    trnspgs: 'tranches_pages',
    trnspg: 'tranches_page_',
    sku: 'sku_token_',
    skuspgs: 'skus_pages_',
    skuspg: 'skus_page_',
    ...
    alw: 'allowance_',
    repn: 'redemption_',
    repnsaltspgs: 'redemptions_applicants_pages',
    repnsaltspg: 'redemptions_applicants_page_',
    altrepnspgs: 'applicant_redemptions_pages_',
    altrepnspg: 'applicant_redemptions_page_',
    dpt: 'dispute_',
    evi: 'evidence_'
};
...
const error = {
    CPY_NAME_ERR: {
        code: 20000,
        msg: 'The companyName must be a string and its length must be between 1 and 1024.'
    },
    CPY_CNT_ERR: {
        code: 20001,
    ...
    SPUS_MORE_ERR: {
        code: 20015,
        msg: 'The pages of all spus cannot be bigger than 2000.'
    },
    SPU_NOT_EST: {
        code: 20016,
    ...
    TRNS_MORE_ERR: {
        code: 20019,
        msg: 'The pages of all tranches cannot be bigger than 2000.'
    },
    TRN_NOT_EST: {
        code: 20020,
    ...
    ABT_ERR: {
        code: 20033,
        msg: 'The abstract must be array, its size must be between 1 and 20 and each length must be between 1 and 64.'
    },
    ACP_ID_ERR: {
        code: 20034,
    ...
    SKUS_MORE_ERR: {
        code: 20036,
        msg: 'The pages of all skus cannot be bigger than 2000.'
    },
    ACP_EST_SKU_ERR: {
        code: 20037,
    ...
    PRD_ERR: {
        code: 20059,
        msg: 'The period must be a string and its length must be between 1 and 16.'
    },
    ACP_NOT_EST: {
        code: 20060,
    ...
    REPN_ALT_MORE: {
        code: 20062,
        msg: 'The pages of all redemption applicants in sku cannot be bigger than 2000.'
    },
    ALT_REPN_MORE: {
        code: 20063,
        msg: 'The pages of the appicant redemptions in sku cannot be bigger than 2000.'
    },
    APT_ERR: {
        code: 20064,
...
function _assert(_cdn, _err) {
    Utils.assert(_cdn, _err);
}

function _intCheck(_pa) {
    return Utils.stoI64Check(_pa);
}

function _intAdd(_pa1, _pa2) {
    return Utils.int64Add(_pa1, _pa2);
}

function _intSub(_pa1, _pa2) {
    return Utils.int64Sub(_pa1, _pa2);
}

function _intMul(_pa1, _pa2) {
    return Utils.int64Mul(_pa1, _pa2);
}

function _intCompare(_pa1, _pa2) {
    return Utils.int64Compare(_pa1, _pa2);
}
...
function _checkInt(_var, _min, _max) {
    return Utils.stoI64Check(_var) && _intCompare(_var, _min) >= 0 && _intCompare(_var, _max) <= 0;
}
...
function _checkAddr(_addr) {
    return Utils.addressCheck(_addr) && _intCompare(Chain.getBalance(_addr), 0) > 0;
}

function _checkExpired(_stTm) {
    const sevenDaysMs = _intMul(_intMul(_intMul(7, 24), 3600), 1000000);
    const curDaysMs = _intSub(Chain.block.timestamp, _stTm);
    return _intCompare(curDaysMs, sevenDaysMs) > 0;
}

function _checkExist(_key, _errCode) {
    const value = _load(_key);
    _assert(value !== false, _throwErr(_errCode));
...
function _checkTranche(_trnId, _dftTrnId, _isDft) {
    if (_trnId === undefined) {
        ...
        _assert(_checkStr(_trnId, 1, 32), _throwErr(error.TRN_ID_ERR));
        if (_isDft === true) {
            // If the sku has default tranche, checking whether the tranche is default tranche.
            _assert(_dftTrnId === undefined || _trnId === _dftTrnId, _throwErr(error.TRN_NOT_DFT));
        } else if (_isDft === false) {
            // If the sku has not default tranche, checking whether the tranche is not default tranche.
            _assert(_dftTrnId === undefined || _trnId !== _dftTrnId, _throwErr(error.TRN_DFT));
        }
    }
    return _trnId;
}

function _checkSkuTranche(_isDftTrn, _skuId, _trnId) {
    ...
    if (_trnId === undefined) {
        _trnId = skuTk.trancheId;
    } else {
        _assert(_checkStr(_trnId, 1, 32), _throwErr(error.TRN_ID_ERR));
        _assert(_isDftTrn !== true || _trnId === skuTk.trancheId, _throwErr(error.TRN_ID_NOT_DFT));
    }
    return _trnId;
}

function _setSeller(_sender, _cpyNm, _cpyCat, _attrs) {
    // Checking parameters.
    _assert(_checkStr(_cpyNm, 1, 1024), _throwErr(error.CPY_NAME_ERR));
    _assert(_checkStr(_cpyCat, 1, 64), _throwErr(error.CPY_CNT_ERR));

    // Storing the seller.
    const seller = {};
    seller.address = _sender;
    seller.companyName = _cpyNm;
...
function _checkArrayItem(_arr, _minLen, _maxLen, _itemName, _err) {
    if (_arr !== undefined) {
        ...
        let i = 0;
        for (i = 0; i < len; i += 1) {
            _assert(_checkStr(_arr[i], _minLen, _maxLen), _throwErrMsg(_err.code, `The ${_itemName} of index ${i} is not between ${_minLen} and ${_maxLen}.`));
        }
    }
}
...
function _checkAddArrayItem(_isAdd, _id, _key, _pa1, _pa2, _pa3, _pa4, _idx) {
    ...
    if (val !== false) {
        const maxLen = "256000";
        const len = _intAdd(val.length, _id.length);
        if (_intCompare(len, maxLen) > 0) {
            return -2;
        }
...
/**
 * 检测并添加元素到某页的列表中
 * @return -2: 添加元素时但，页数也增加，是超过了2000。
 *         -1: 检测已存在该元素
...
/**
 * 检测并添加元素到页中
 * @return -2: 添加元素时，页数也增加，但是超过了2000。
 *         -1: 检测已存在该元素
 *          0: 检测不存在该元素，并且添加该元素成功
 *        > 0: 添加到元素到新页中，并返回新页数
 */
...
function _checkAddBalance(_to, _skuId, _trnId, _val) {
    ...
    const skuAddRet = _addVal(_val, keys.ble, _skuId, _to);
    if (skuAddRet) {
        _checkAddPagesItem(true, _skuId, keys.skuspgs, keys.skuspg, _to);
    }
...
function _approve(_spr, _skuId, _trnId, _val) {
    ...
    _assert(Utils.stoI64Check(_val) && _intCompare(_val, 0) > 0, _throwErr(error.VAL_ERR));
...
function _checkSubBalance(_sender, _frm, _skuId, _trnId, _val, _isSub, _isDftTrn) {
    ...
    _assert(trnSubRet !== -1, _throwErr(error.BLE_NOT_EGH));
...
function _transfer(_frm, _skuId, _trnId, _to, _val) {
    // Checking parameters.
    _assert(_checkAddr(_frm), _throwErr(error.FRM_ERR));
    _assert(_checkAddr(_to), _throwErr(error.TO_ERR));
    _assert(_checkStr(_skuId, 1, 32), _throwErr(error.SKU_ID_ERR));
    _assert(Utils.stoI64Check(_val) && _intCompare(_val, 0) > 0, _throwErr(error.VAL_ERR));
...
function _destroy(_addr, _skuId, _trnId, _val) {
    ...
    skuTk.totalSupply = _intSub(skuTk.totalSupply, _val);
...
function setDocument(id, name, url, hashType, hash) {
    // Checking parameters.
    _assert(_checkStr(id, 1, 32), _throwErr(error.DOC_ID_ERR));
    _assert(_checkStr(name, 1, 1024), _throwErr(error.NAME_ERR));
    _assert(_checkStr(url, 1, 10240), _throwErr(error.DOC_URL_ERR));
    _assert(_checkStr(hashType, 1, 64), _throwErr(error.HASH_TYPE_ERR));
    _assert(_checkStr(hash, 1, 2048), _throwErr(error.HASH_ERR));

    // Checking whether the provider is seller.
    const isSel = _checkIsSeller(gTxSender);
    _assert(isSel !== false, _throwErr(error.NOT_SEL));

    // Checking whether all documents are more.
    _assert(_checkAddPagesItem(true, id, keys.docspgs, keys.docspg) !== -2, _throwErr(error.DOCS_MORE_ERR));
...
function documentInfo(docId) {
    // Checking parameters.
    _assert(_checkStr(docId, 1, 32), _throwErr(error.DOC_ID_ERR));
...
function createSpu(id, name, type, attrs) {
    // Checking parameters.
    _assert(_checkStr(id, 1, 32), _throwErr(error.SPU_ID_ERR));
    _assert(_checkStr(name, 1, 1024), _throwErr(error.NAME_ERR));
    _assert(_checkStr(type, 1, 64), _throwErr(error.SPU_TYPE_ERR));

    // Checking whether the sender is seller.
    _assert(_checkIsSeller(gTxSender), _throwErr(error.NOT_SEL));

    // Checking whether the spu does not exist.
    const spuKey = _makeKey(keys.spu, id);
    _checkNotExist(spuKey, error.SPU_EST);

    // Checking whether all spus are more.
    _assert(_checkAddPagesItem(true, id, keys.spuspgs, keys.spuspg) !== -2, _throwErr(error.SPUS_MORE_ERR));
...
function allSpus() {
    // Getting the documents.
    const spus = _getPagesItems(keys.spuspgs, keys.spuspg);
...
function createTranche(id, des, lms) {
    // Checking parameters.
    _assert(_checkStr(id, 1, 32), _throwErr(error.TRN_ID_ERR));
    _assert(_checkStr(des, 0, 64000), _throwErr(error.DES_ERR));
    ...
    // Checking whether all tranches are more.
    _assert(_checkAddPagesItem(true, id, keys.trnspgs, keys.trnspg) !== -2, _throwErr(error.TRNS_MORE_ERR));
...
function allTranches() {
    // Getting the documents.
    const trns = _getPagesItems(keys.trnspgs, keys.trnspg);
...
function trancheInfo(trnId) {
    // Checking parameters.
    _assert(_checkStr(trnId, 1, 32), _throwErr(error.TRN_ID_ERR));
...
function issue(skuId, trnId, isDftTrn, spuId, name, symbol, supply, decimals, mainIcn, viceIcns, labels, des, repnAddr, acpId, abs, attrs) {
    ...
    _assert(_checkStr(skuId, 1, 32), _throwErr(error.SKU_ID_ERR));
    _assert(isDftTrn === undefined || typeof isDftTrn === 'boolean',_throwErr(error.IS_DFT_TRN_ERR));
    _assert(_checkStr(spuId, 0, 32), _throwErr(error.SPU_ID_ERR));
    _assert(_checkStr(name, 1, 1024), _throwErr(error.NAME_ERR));
    _assert(_checkStr(symbol, 1, 16), _throwErr(error.TK_BML_ERR));
    _assert(Utils.stoI64Check(supply) && _intCompare(supply, 0) > 0, _throwErr(error.TK_SPY_ERR));
    _assert(_checkInt(decimals, 0, 8), _throwErr(error.TK_DCS_ERR));
    _assert(_checkStr(mainIcn, 0, 10240), _throwErr(error.MAIN_ICN_ERR));
    _assert(_checkJSNArr(viceIcns, 0, 5), _throwErr(error.VICE_ICNS_ERR));
    _assert(_checkStr(des, 0, 64000), _throwErr(error.DES_ERR));
    _assert(_checkJSNArr(labels, 0, 20), _throwErr(error.LBL_ERR));
    _assert(_checkAddr(repnAddr), _throwErr(error.REPN_ADDR_ERR));
    _assert(_checkJSNArr(abs, 0, 20), _throwErr(error.ABT_ERR));
    _assert(_checkStr(acpId, 1, 64), _throwErr(error.ACP_ID_ERR));
    ...
    // Checking whether the count of skus is more.
    _assert(_checkAddPagesItem(true, skuId, keys.skuspgs, keys.skuspg, gTxSender) !== -2, _throwErr(error.SKUS_MORE_ERR));
...
function setSkusChoice(spuId, choice) {
    // Checking parameters.
    _assert(_checkStr(spuId, 1, 32), _throwErr(error.SPU_ID_ERR));
    _assert(_checkJSNObj(choice) && _checkStr(_toStr(choice), 1, 1024), _throwErr(error.COC_ERR));
...
function setSku(skuId, name, symbol, mainIcn, viceIcns, labels, des, repnAddr, abs, attrs) {
    // Checking parameters.
    _assert(_checkStr(skuId, 1, 32), _throwErr(error.SKU_ID_ERR));
    _assert(_checkStr(name, 0, 1024), _throwErr(error.NAME_ERR));
    _assert(_checkStr(symbol, 0, 16), _throwErr(error.TK_BML_ERR));
    _assert(_checkStr(mainIcn, 0, 10240), _throwErr(error.MAIN_ICN_ERR));
    _assert(_checkJSNArr(viceIcns, 0, 5), _throwErr(error.VICE_ICNS_ERR));
    _assert(_checkStr(des, 0, 64000), _throwErr(error.DES_ERR));
    _assert(_checkJSNArr(labels, 0, 20), _throwErr(error.LBL_ERR));
    _assert(repnAddr === undefined || _checkAddr(repnAddr), _throwErr(error.REPN_ADDR_ERR));
    _assert(_checkJSNArr(abs, 0, 20), _throwErr(error.ABT_ERR));
...
function addAcceptanceToSku(skuId, acpId) {
    ...
    _assert(ret !== -1, _throwErr(error.ACP_EST_SKU_ERR));
    _assert(ret !== -2, _throwErr(error.ACP_MORE_SKU_ERR));
...
function delAcceptanceFromSku(skuId, acpId) {
    ...
    _assert(ret !== -1, _throwErr(error.ACP_NOT_EST_SKU_ERR));
...
function additionalIssuance(skuId, trnId, supply) {
    ...
    skuTk.totalSupply = _intAdd(skuTk.totalSupply, supply);
    _store(skuTkKey, _toStr(skuTk));
...
function assignToTranche(skuId, toTrnId, val) {
    // Checking parameters.
    _assert(_checkStr(skuId, 1, 32), _throwErr(error.SKU_ID_ERR));
    _assert(_checkStr(toTrnId, 1, 32), _throwErr(error.TRN_ID_ERR));
    _assert(Utils.stoI64Check(val) && _intCompare(val, 0) > 0, _throwErr(error.VAL_ERR));

    // Checking whether the sender is seller.
    const isSel = _checkIsSeller(gTxSender);
    _assert(isSel, _throwErr(error.NOT_SEL));
    ...
    _assert(skuTk.defaultTrancheId !== undefined, _throwErr(error.SKU_NO_DFT_TRN));

    // Checking whether the target tranche id exists and it is not default tranche.
    const dftTrnId = skuTk.defaultTrancheId;
    _assert(dftTrnId !== toTrnId, _throwErr(error.TO_TRN_DEFAULT_ERR));
...
function setAuthorizers(skuId, autrs) {
    // Checking parameters.
    _assert(_checkStr(skuId, 1, 32), _throwErr(error.SKU_ID_ERR));
    _assert(_checkJSNArr(autrs, 1, 10), _throwErr(error.ATOS_ERR));
    ...
    for(i = 0; i < len; i += 1) {
        const autr = autrs[i];
        _assert(_checkAddr(autr), _throwErrMsg(error.ATOS_ERR, `The autorizer of index ${i} is invalid.`));
    }
...
function authorizeSku(skuId, trnId) {
    ...
    _assert(idx !== -1, _throwErr(error.ATOR_NOT_EST));
    ...
    _assert(trnInSku !== 0, _throwErr(error.TRN_NOT_IN_SKU));
 ...
 * @return {string}
 */
function skusOf(addr) {
    // Checking parameters.
    _assert(_checkAddr(addr), _throwErr(error.ADDR_ERR));

    // Getting the skus.
    const skus = _getPagesItems(keys.skuspgs, keys.skuspg, addr);
...
/**
 * 返回指定 SKU 的所有承兑方
 * @param {string} acpId [承兑方编号]
 * @return {string}
 * @throws {error}
 */
function acceptancesOfSku(acpId) {
    // Checking parameters.
    _assert(_checkStr(acpId, 1, 32), _throwErr(error.ACP_ID_ERR));
    ...
    // Getting the tranches of sku.
    const acps = _getPagesItems(keys.skuacpspgs, keys.skuacpspg, acpId);
...
function balanceOf(address, skuId) {
    // Checking parameters.
    _assert(_checkStr(skuId, 1, 32), _throwErr(error.SKU_ID_ERR));
    _assert(_checkAddr(address), _throwErr(error.ADDR_ERR));
...
function destroy(address, skuId, trnId, val) {
    ...
    // Checking whether the address is seller.
    _assert(address === gTxSender, _throwErr(error.SEL_RDM_OTR));

    // Checking the sku and tranche
    trnId = _checkSkuTranche(skuId, trnId);
    _checkExist(_makeKey(keys.trn, trnId), error.TRN_NOT_EST);
...
function allowance(owr, skuId, trnId, spr) {
    // Checking parameters.
    _assert(_checkAddr(owr), _throwErr(error.ONR_ERR));
    _assert(_checkStr(skuId, 1, 32), _throwErr(error.SKU_ID_ERR));
    _assert(_checkAddr(spr), _throwErr(error.SPR_ERR));
 ...
 * @throws {error}
 */
function setAcceptance(id, addr, name, logo, cat, period, addi) {
    // Checking parameters.
    _assert(_checkStr(id, 1, 32), _throwErr(error.ACP_ID_ERR));
    _assert(_checkAddr(addr), _throwErr(error.ADDR_ERR));
    _assert(_checkStr(name, 1,1024), _throwErr(error.NAME_ERR));
    _assert(_checkStr(logo, 0, 10240), _throwErr(error.LOGO_ERR));
    _assert(_checkStr(cat, 1, 64), _throwErr(error.CAT_ERR));
    _assert(_checkStr(period, 1, 16), _throwErr(error.PRD_ERR));
...
function requestRedemption(repnId, skuId, trnId, val, acpId, des, addi) {
    // Checking parameters.
    _assert(_checkStr(repnId, 1, 32), _throwErr(error.REPN_ID_ERR));
    _assert(_checkStr(skuId, 1, 32), _throwErr(error.SKU_ID_ERR));
    _assert(_checkStr(trnId, 0, 32), _throwErr(error.TRN_ID_ERR));
    _assert(Utils.stoI64Check(val) && _intCompare(val, 0) > 0, _throwErr(error.VAL_ERR));
    _assert(_checkStr(acpId, 1, 32), _throwErr(error.ACP_ID_ERR));
    _assert(_checkStr(des, 0, 64000), _throwErr(error.DES_ERR));
    ...
    // Checking whether the from balance is enough.
    const trns = _checkSubBalance(gMsgSender, gMsgSender, skuId, trnId, val);

    // Adding the redemption applicant, and Checking whether the count is more.
    _assert(_checkAddPagesItem(true, gMsgSender, keys.repnsaltspgs, keys.repnsaltspg) !== -2, _throwErr(error.REPN_ALT_MORE));

    // Adding the redemption, and checking whether the count is more.
    _assert(_checkAddPagesItem(true, repnId, keys.altrepnspgs, keys.altrepnspg, gMsgSender) !== -2, _throwErr(error.ALT_REPN_MORE));
...
function redeem(repnId, apt) {
    // Checking parameters.
    _assert(_checkStr(repnId, 1, 32), _throwErr(error.REPN_ID_ERR));
    _assert(_checkAddr(apt), _throwErr(error.APT_ERR));
    ...
    _assert(repn.status === 0, _throwErr(error.REPN_SAS_ERR));
    ...
    _assert(gTxSender === acp.address, _throwErr(error.NOT_APR));
...
function confirmRedemption(repnId, apt) {
    ...
    const repnVal = _checkExist(repnKey, error.REPN_NOT_EST);
    let repn = _toJsn(repnVal);
    _assert(repn.status === 1, _throwErr(error.REPN_SAS_ERR));

    // Checking whether the redemption is expired.
    const isExired = _checkExpired(repn.sendTime);

    // Checking whether the sender is applicant
    if (!isExired) {
        _assert(gMsgSender === apt, _throwErr(error.NOT_APT));
    }
 ...
 * @return {string}
 */
function allApplicantsOfRedemptions() {
    // Getting the documents.
    const alts = _getPagesItems(keys.repnsaltspgs, keys.repnsaltspg);
 ...
 * @param {string} apt [申请人区块链账户地址]
 */
function redemptionsOf(apt) {
    // Checking parameters.
    _assert(_checkAddr(apt), _throwErr(error.APT_ERR));

    // 获取申请人的所有兑付的id列表
    const repns = _getPagesItems(keys.altrepnspgs, keys.altrepnspg, apt);
...
function applyDispute(repnId, apt, rsn, ctr, addi) {
    ...
    _assert(_checkStr(repnId, 1, 32), _throwErr(error.REPN_ID_ERR));
    _assert(_checkAddr(apt), _throwErr(error.APT_ERR));
    _assert(_checkStr(rsn, 1, 64000), _throwErr(error.RSN_ERR));
    _assert(_checkAddr(ctr), _throwErr(error.CTR_ERR));

    // Checking the sender is seller or redemption applicant.
    _assert(_checkIsSeller(gMsgSender) || gMsgSender === apt, _throwErr(error.NOT_SEL_APT));
    ...
    _assert(repn.status === 0 || repn.status === 1, _throwErr(error.SAS_FOR_DPT_ERR));
...
function setEvidence(repnId, apt, des, addi) {
    ...
    const acpVal = _load(acpKey);
    const acp = _toJsn(acpVal);
    _assert(gMsgSender === apt || _checkIsSeller(gMsgSender) || gMsgSender === acp.address, _throwErr(error.NOT_APT_SEL_APR));
...
function evidenceInfo(repnId, apt, pvr) {
    ...
    _assert(_checkStr(repnId, 1, 32), _throwErr(error.REPN_ID_ERR));
    _assert(_checkAddr(apt), _throwErr(error.APT_ERR));
    _assert(_checkAddr(pvr), _throwErr(error.PVR_ERR));
...
function handleDispute(repnId, apt, sas, des) {
    ...
    _assert(_checkStr(repnId, 1, 32), _throwErr(error.REPN_ID_ERR));
    _assert(_checkAddr(apt), _throwErr(error.APT_ERR));
    _assert(sas === 1 || sas === 2, _throwErr(error.SAS_ERR));
    _assert(_checkStr(des, 0, 64000), _throwErr(error.DES_ERR));
    ...
    _assert(dpt.controller === gMsgSender, _throwErr(error.SENDER_NOT_CTR));

    // Checking whether the dispute status is 0.
    _assert(dpt.status === 0, _throwErr(error.DPT_FSH));
...
/**
 * 设置商家信息
 * @param {string} cpyNm [公司名称]
 * @param {string} cpyCat [公司联系方式]
 * @param {Object} attrs [属性]
 * @throws {error}
 */
function setSeller(cpyNm, cpyCat, attrs) {
    // Checking whether the sender is seller.
    const isSel = _checkIsSeller(gTxSender);
    _assert(isSel, _throwErr(error.NOT_SEL));

    // Setting the seller.
    _setSeller(gTxSender, cpyNm, cpyCat, attrs);

    // Committing event.
    Chain.tlog('setSeller', _makeTlogSender(), cpyNm, cpyCat);
}
...
function init(bar) {
    ...
    // Setting the seller.
    _setSeller(gTxSender, params.companyName, params.companyContact, params.attributes);
    ...
    // Committing event.
    Chain.tlog('init', _makeTlogSender(), params.companyName, params.companyContact);
}
...
function main(input) {
    ...
    switch (method) {
        case 'setSeller':
            setSeller(params.companyName, params.companyContact, params.attributes);
            break;
        case 'setDocument':
        ...
        case 'setSkusChoice':
            ...
            break;
        case 'setSku':
            setSku(params.skuId, params.name, params.symbol, params.mainIcon, params.viceIcons, params.labels, params.description, params.redemptionAddress, params.abstract, params.attributes);
            break;
        ...
        case 'additionalIssuance':
            additionalIssuance(params.skuId, params.trancheId, params.supply, params.information);
            break;
        case 'assignToTranche':
            assignToTranche(params.skuId, params.toTrancheId, params.value, params.defaultTokensInformation, params.toTokensInformation);
            break;
        case 'setAuthorizer':
            setAuthorizers(params.skuId, params.authorizers);
            break;
        case 'authorizeSku':
            authorizeSku(params.skuIds);
            break;
        case 'createTranche':
...
function query(input) {
    ...
    switch (method) {
        ...
        case 'balanceOfByTranche':
            ...
            break;
        case 'allowance':
            val = allowance(params.owner, params.skuId, params.spender);
            break;
        case 'allowanceByTranche':
            val = allowance(params.owner, params.skuId, params.trancheId, params.spender);
            break;
        case 'allApplicantsOfRedemptions':
            val = allApplicantsOfRedemptions();
            break;
        case 'redemptionInfo':
