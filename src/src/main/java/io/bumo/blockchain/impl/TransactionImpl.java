...
import io.bumo.account.impl.AccountImpl;
import io.bumo.asset.impl.AssetImpl;
import io.bumo.asset.impl.BUImpl;
import io.bumo.asset.impl.TokenImpl;
import io.bumo.blockchain.Transaction;
import io.bumo.common.General;
import io.bumo.contract.impl.ContractImpl;
        ...
        for (int i = 0; i < operationBase.length; i++) {
            ...
            switch (operationBase[i].getOperationType()) {
                case ACCOUNT_ACTIVATE:
                    operation = AccountImpl.activate((AccountActiviateOperation)operationBase[i]);
                    break;
                case ACCOUNT_SET_METADATA:
                    operation = AccountImpl.setMetadata((AccountSetMetadataOperation) operationBase[i]);
                    break;
                case ACCOUNT_SET_PRIVILEGE:
                    operation = AccountImpl.setPrivilege((AccountSetPrivilegeOperation) operationBase[i]);
                    break;
                case ASSET_ISSUE:
                    operation = AssetImpl.issue((AssetIssueOperation) operationBase[i]);
                    break;
                case ASSET_SEND:
                    operation = AssetImpl.send((AssetSendOperation) operationBase[i]);
                    break;
                case BU_SEND:
                    operation = BUImpl.send((BUSendOperation) operationBase[i]);
                    break;
                case TOKEN_ISSUE:
                    operation = TokenImpl.issue((TokenIssueOperation) operationBase[i]);
                    break;
                case TOKEN_TRANSFER:
                    operation = TokenImpl.transfer((TokenTransferOperation) operationBase[i]);
                    break;
                case TOKEN_TRANSFER_FROM:
                    operation = TokenImpl.transferFrom((TokenTransferFromOperation) operationBase[i]);
                    break;
                case TOKEN_APPROVE:
                    operation = TokenImpl.approve((TokenApproveOperation) operationBase[i]);
                    break;
                case TOKEN_ASSIGN:
                    operation = TokenImpl.assign((TokenAssignOperation) operationBase[i]);
                    break;
                case TOKEN_CHANGE_OWNER:
                    operation = TokenImpl.changeOwner((TokenChangeOwnerOperation) operationBase[i]);
                    break;
                case CONTRACT_CREATE:
                    operation = ContractImpl.create((ContractCreateOperation) operationBase[i]);
                    break;
                case CONTRACT_INVOKE_BY_ASSET:
                    operation = ContractImpl.invokeByAsset((ContractInvokeByAssetOperation) operationBase[i]);
                    break;
                case CONTRACT_INVOKE_BY_BU:
                    operation = ContractImpl.invokeByBU((ContractInvokeByBUOperation) operationBase[i]);
                    break;
            }
